...
test('setValue writes the AC mode and the scaled setpoint to the cloud', async () => {
  ...
  assert.deepEqual(commands, [
    // PATCH (local fork, not upstream, see src/tuya/device/tuya.deviceMapping.js):
    // GLADYS_AC_MODE_TO_TUYA / GLADYS_AC_FAN_SPEED_TO_TUYA write the exact
    // uppercase words / literal digit strings the real MEDION P502 (MD37735)
    // expects, not the generic lowercase vocabulary this fixture's device
    // reports on READ. Confirmed via Tuya IoT Platform > Device Debugging.
...
test('poll restores the scale lost by Gladys persistence (cloud read)', async () => {
  ...
  // PATCH (local fork, not upstream, see src/devices/airConditioner.js):
  // the fallback scale for temp_set/temp_current was changed from 1 to 0 —
  // the real MEDION P502 (MD37735) reports "scale":0 in its own Tuya
  // schema, confirmed via Tuya IoT Platform > Device Debugging. With
  // scale 0 there is no /10 division: raw device values pass through as-is.
  const states = Object.fromEntries(fake.published.map((p) => [p.featureExternalId, p.state]));
  assert.equal(states['ext:tuya:device:ac1:temp_set'], 240);
  assert.equal(states['ext:tuya:device:ac1:temp_current'], 230);
});
...
test('setValue restores the scale lost by Gladys persistence', async () => {
  ...
  // With the patched scale of 0 (see the comment above), a Gladys value of
  // 24 is sent to the device as 24, not multiplied by 10 into 240.
  await handler.setValue(device, feature('temp_set'), 24);
  assert.deepEqual(commands, [{ code: 'temp_set', value: 24 }]);
});
